...
public class KVQuery extends HibernateQuery {
	...
	private static final String SQL_FIND_ALL_KV_BY_ALL = 
			"select distinct kv.kv_serial_no, kind.kind_name kind, kv.name name, kv.img_path, kv.click_link, kv.msg, map.start_date, map.end_date, map.isEnabled, kv.type "
			+ " from kv kv "
	...
	private static final String SQL_ADD_NEW_KV = "insert into kv (kind, img_path, click_link, msg, create_date, create_user, name, type) values (?, ?, ?, ?, ?, ?, ?, ?)"; 
	
	private static final String SQL_UPDATE_KV = "update kv set img_path = ?, click_link = ?, msg = ?, kind = ?, update_date = ?, update_user = ?, name = ?, type = ? where kv_serial_no = ?";
	
	private static final String SQL_FIND_KV_BY_MACHINE = 
			"select kv.kv_serial_no, kv.kind, kv.name, kv.img_path, kv.click_link, kv.msg, kcm.start_date, kcm.end_date, kcm.isEnabled isEnabled, kv.type " + 
			"	from kv kv  " + 
	 ...
	 * @param user
	 */
	public void updateKV(int serialNo, int kind, String imgPath, String clickLink, String msg, String user, String kvName, String type) {
		Profiler p = new Profiler();
		log.trace("START: {}.updateKV(), kind: {}, imgPath: {}, clickLink: {}, msg: {}, user: {}, serialNo: {}, type: {}", this.getClass(), kind, imgPath, clickLink, msg, user, serialNo, type);
		super.executeUpateQuery(SQL_UPDATE_KV, imgPath, clickLink, msg, kind, new Timestamp(System.currentTimeMillis()), user, kvName, type, serialNo);
		log.info("END: {}.updateKV(), kind: {}, imgPath: {}, clickLink: {}, msg: {}, user: {}, serialNo: {}, type: {}, exec TIME: {} ms.", this.getClass(), kind, imgPath, clickLink, msg, user, serialNo, type, p.executeTime());
	}
	...
	public int addNewKV(int kind, String imgPath, String clickLink, String msg, String user, String kvName, String type) {
		Profiler p = new Profiler();
		log.trace("START: {}.addNewKV(), kind: {}, imgPath: {}, clickLink: {}, msg: {}, type: {}, user: {}", this.getClass(), kind, imgPath, clickLink, msg, type, user);
		super.executeUpateQuery(SQL_ADD_NEW_KV, kind, imgPath, clickLink, msg, new Timestamp(System.currentTimeMillis()), user, kvName, type);
